...
const NebulaScene = ({ distance, pinchL, pinchR }: { distance: number, pinchL: number, pinchR: number }) => {
  ...
  const stateRef = useRef({
    w_fusion: 0, v_fusion: 0,
    w_repel: 0, v_repel: 0,
    w_collapse: 0, v_collapse: 0,
    w_neutral: 1, v_neutral: 0,
    shockwave: 0
  });
  ...
  const { positions, colors, velocities, baseData } = useMemo(() => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      if (type === 0) {
        ...
        baseHue = 0.80 + (Math.random() - 0.5) * 0.05;
        baseSat = 0.8;
        baseLight = 0.5;
      } else {
  ...
  useFrame((state, delta) => {
    ...
    const totalDelta = Math.abs(target_fusion - s.w_fusion) + 
                       Math.abs(target_repel - s.w_repel) + 
                       Math.abs(target_collapse - s.w_collapse);
